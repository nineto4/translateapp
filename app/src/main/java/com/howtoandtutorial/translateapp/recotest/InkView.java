...
package com.howtoandtutorial.translateapp.recotest;
...
import com.howtoandtutorial.translateapp.TapTapActivity;
import com.phatware.android.RecoInterface.WritePadAPI;
import com.howtoandtutorial.translateapp.write.WritePadManager;

import java.util.LinkedList;

public class InkView extends View implements TapTapActivity.OnInkViewListener{
    private Path mPath;
...
//                    }
                    edtInput.append(WritePadManager.recoResultWord(w, 0)+" ");
                }
            }
        }
    };
    ...
    public InkView(Context context, AttributeSet attrs, int defStyle) {
        ...
        Log.d(Common.TAG,"InkView -> constructer ");
        mPath = new Path();
    ...
    protected void onDraw(Canvas canvas) {
        mPaint.setColor(0xFFFF0000);//->FF0000 == red
        Log.d(Common.TAG,"InkView -> onDraw() ");
        //ve o ly tren canvas
//      mPaint.setStrokeWidth(1);// do rong khi ve
...
//		}
        ...
        canvas.drawColor(0xFFFFFFFF); //->FFFFFF == white
    ...
    private void touch_start(float x, float y) {
        Log.d(Common.TAG,"InkView -> touch_start() ");
        mPath.reset();
        mPath.moveTo(x, y);
        mX = x;
        mY = y;
    ...
    private void touch_move(float x, float y) {
        Log.d(Common.TAG,"InkView -> touch_move() ");
    ...
    private void touch_up() {
        // stopRecognizer();
        Log.d(Common.TAG,"InkView -> touch_up() ");
        mCurrStroke = -1;
        if (!mMoved)
            mX++;
        mMoved = false;
        ...
        TapTapActivity taptap = (TapTapActivity) getContext();
            ...
            if (gesturetype != WritePadAPI.GEST_NONE && gesturetype != WritePadAPI.GEST_BACK) {
                ...
                switch (gesturetype) {
                    ...
                    case WritePadAPI.GEST_BACK_LONG:
                        ...
                        taptap.mBoundService.dataNotify(WritePadManager.recoStrokeCount());
                        return;
        ...
        taptap.mBoundService.dataNotify(nStrokeCnt);
    }
...
//        }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        float x = event.getX();
        float y = event.getY();
        Log.d(Common.TAG,"InkView -> onTouchEvent() ");
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                Log.d(Common.TAG,"InkView -> ACTION_DOWN() ");
                touch_start(x, y);
                invalidate();
                break;
    ...
    public void setEdtInput(EditText edtInput) {
